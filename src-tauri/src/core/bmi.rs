...
- BMI value
- BMI category
Each patient prompt will include these params, your response must be structured as a stringified JSON object with the following fields:
...
- be deterministic with respect to the input parameters.
- The strings in the field MUST not contain any markdown or effects (e.g: bold, italic...)"#;
...
pub async fn get_feedback(llm: &dyn LlmProvider, bmi_input: &BmiInput) -> Result<BmiFeedback, Box<dyn Error>> {
    let data = format!(
